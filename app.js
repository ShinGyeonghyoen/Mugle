...
const celebrateCharacter = document.getElementById("celebrateCharacter");
const celebrateEyebrow = document.getElementById("celebrateEyebrow");
const celebrateMenuName = document.getElementById("celebrateMenuName");
const celebrateMessage = document.getElementById("celebrateMessage");
const celebrateShareBtn = document.getElementById("celebrateShareBtn");
const celebrateCloseBtn = document.getElementById("celebrateCloseBtn");
const celebrateRetryBtn = document.getElementById("celebrateRetryBtn");
...
const celebrateEyebrowMessages = [
  "모구가 신나서 춤춘다",
  "결정 완료! 이제 먹으러 가자",
  "오늘 점심 미션 클리어",
  "드디어 메뉴가 확정됐다",
...
const celebrateBodyMessages = [
  (menu) => `좋아, ${menu.name}로 확정! 오늘 점심은 이걸로 가자.`,
  (menu) => `${menu.name} 결정 완료. 모구가 아주 흡족해하고 있다.`,
  (menu) => `오늘의 메뉴는 ${menu.name}! 이제 맛있게 먹으러 가면 된다.`,
  (menu) => `${menu.name}(으)로 끝냈다. 고민은 여기서 종료다.`,
  (menu) => `${menu.name} 당첨. 모구의 춤을 봤으면 이제 실행만 남았다.`
];
...
function hideShareButtons() {
  singleShareWrap?.classList.add("hidden");
  tripleShareWrap?.classList.add("hidden");
}

function showSingleShareButton() {
  if (isSharedSession || isReplySession) return;
  singleShareWrap?.classList.remove("hidden");
  tripleShareWrap?.classList.add("hidden");
}

function showTripleShareButton() {
  if (isSharedSession || isReplySession) return;
  tripleShareWrap?.classList.remove("hidden");
  singleShareWrap?.classList.add("hidden");
}

function hideRelayButton() {
  relayResultWrap?.classList.add("hidden");
}

function showRelayButton() {
  relayResultWrap?.classList.remove("hidden");
}

function hideFriendResultButtons() {
  friendResultWrap?.classList.add("hidden");
}

function showFriendResultButtons() {
  friendResultWrap?.classList.remove("hidden");
}
...
/* =========================
   Celebrate Overlay Helpers
========================= */
function randomPick(arr) {
  return arr[Math.floor(Math.random() * arr.length)];
}

function getCelebrateMessage(menu) {
  return randomPick(celebrateBodyMessages)(menu);
}

function showCelebrateOverlay(menu, options = {}) {
  if (!celebrateOverlay || !menu) return;

  const eyebrowText = options.eyebrow || randomPick(celebrateEyebrowMessages);
  const messageText = options.message || getCelebrateMessage(menu);

  if (celebrateEyebrow) {
    celebrateEyebrow.textContent = eyebrowText;
  }

  if (celebrateMenuName) {
    celebrateMenuName.textContent = `오늘의 메뉴는 ${menu.name}!`;
  }
...
function buildCelebrateShareText(menu) {
  if (!menu) return "Mugle에서 오늘 메뉴를 골랐다.";
  return `Mugle에서 오늘의 메뉴를 ${menu.name}(으)로 결정했다.`;
}

async function shareCelebrateCard() {
  const targetMenu = currentSimpleMenu || sharedSelectedMenu;
  if (!targetMenu) return;

  const shareData = {
    title: `Mugle - 오늘의 메뉴는 ${targetMenu.name}!`,
    text: buildCelebrateShareText(targetMenu),
  ...
  try {
    if (navigator.share) {
      ...
      if (celebrateMessage) {
        celebrateMessage.textContent = "카드용 문구와 링크를 복사했다. 이제 자랑하면 된다.";
      }
    } else {
      window.prompt("이 내용을 복사해서 공유해줘", `${shareData.text}\n${shareData.url}`);
    }
  } catch (error) {
    // 공유 취소 시 무시
  }
...
function retryFromCelebrateCard() {
  ...
  const loadingText = annoyedText || "좋다. 다시 한 번 새 후보를 찾아본다.";
...
function playCelebrateSequence(menu, options = {}) {
  playHighlightEffect();

  setTimeout(() => {
    showCelebrateOverlay(menu, options);
  }, 520);
}
...
function activateMode(mode) {
  ...
  if (mode === "simple") {
    ...
    if (messageBubble) messageBubble.textContent = "빠르게 하나 딱 골라줄게!";
  } else {
    tripleModeBtn?.classList.add("active");
    simpleModeBtn?.classList.remove("active");
    if (messageBubble) messageBubble.textContent = "후보 3개를 보여줄 테니 마지막 선택만 해!";
  }
...
function parseShareParams() {
  ...
  if (mode === "single") {
    ...
    if (!item) return null;
    return { mode: "single", item };
  }

  if (mode === "triple") {
    const itemsRaw = params.get("items");
    if (!itemsRaw) return null;
    return { mode: "triple", items: itemsRaw.split("|") };
  }
...
function handleDoomPressStart(event) {
  if (!doomButton || isSharedSession || isReplySession || celebrateOpen) return;
...
celebrateCloseBtn?.addEventListener("click", () => {
  hideCelebrateOverlay();
});

celebrateRetryBtn?.addEventListener("click", () => {
  retryFromCelebrateCard();
});

celebrateShareBtn?.addEventListener("click", async () => {
  await shareCelebrateCard();
});

celebrateOverlay?.addEventListener("click", (event) => {
  if (event.target === celebrateOverlay || event.target.classList.contains("celebrate-backdrop")) {
    hideCelebrateOverlay();
  }
});
